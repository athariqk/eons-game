...
public:
    ...
    Vec2D get_mouse_position() const { return m_mousePosition; }
    Vec2D get_last_mouse_delta() const { return m_lastMouseDelta; }
	Vec2D get_last_mouse_wheel_delta() const { return m_lastMouseWheelDelta; }

private:
    std::unordered_set<KeyboardEvent::Key> m_pressedKeys;
    std::unordered_set<ButtonIndex> m_pressedButtons;
    Vec2D m_mousePosition{0, 0};
    Vec2D m_lastMouseDelta{0, 0};
    Vec2D m_lastMouseWheelDelta{0, 0};
