...
void World::on_fixed_update_(double p_delta, uint64_t ticks) {
    ...
    for (auto i(0u); i < max_groups; i++) {
        ...
        std::erase_if(v, [i](Entity *mEntity) { return !mEntity->is_enabled() || !mEntity->has_group(i); });
    }
    std::erase_if(entities, [](const std::unique_ptr<Entity> &mEntity) { return !mEntity->is_enabled(); });
