...
bool OrganismAISystem::on_init(ncore::World &world) {
    for (const auto &entity_ptr: world.get_entities()) {
        if (!entity_ptr->get_is_enabled() || !entity_ptr->has_component<OrganismAIComponent>())
            continue;
...
void OrganismAISystem::update_run_and_tumble(ncore::Entity &entity, OrganismAIComponent &ai,
                                             OrganismComponent &organism, ncore::World &world, float delta) {
    ai.act_timer += delta;
    ...
    for (auto *food_entity: foods) {
        if (!food_entity->get_is_enabled() || !food_entity->has_component<FoodComponent>())
            continue;
