...
std::unique_ptr<Event> EventBackendSDL::map_from_sdl(SDL_Event &event) {
    switch (event.type) {
        case SDL_EVENT_QUIT:
        case SDL_EVENT_WINDOW_CLOSE_REQUESTED:
            return std::make_unique<QuitEvent>(event.window.windowID);
        case SDL_EVENT_WINDOW_RESIZED:
            return std::make_unique<WindowResizeEvent>(event.window.windowID, event.window.data1, event.window.data2);
...
SDL_Event EventBackendSDL::map_to_sdl(const WindowEvent *event) {
    SDL_Event sdl{};

    switch (event->get_type()) {
        case EventType::KEYBOARD: {
            auto key_ev = dynamic_cast<const KeyboardEvent *>(event);
        ...
        case EventType::MOUSE_BUTTON: {
            auto mb_ev = dynamic_cast<const MouseButtonEvent *>(event);
            sdl.type = mb_ev->action == ButtonAction::PRESS ? SDL_EVENT_MOUSE_BUTTON_DOWN : SDL_EVENT_MOUSE_BUTTON_UP;
        ...
        case EventType::MOUSE_MOTION: {
            auto mm_ev = dynamic_cast<const MouseMotionEvent *>(event);
        ...
        case EventType::MOUSE_WHEEL: {
            auto mw_ev = dynamic_cast<const MouseWheelEvent *>(event);
        ...
        case EventType::TEXT_INPUT: {
            auto ti_ev = dynamic_cast<const TextInputEvent *>(event);
        ...
        case EventType::WINDOW_RESIZE: {
            auto wr_ev = dynamic_cast<const WindowResizeEvent *>(event);
        ...
        case EventType::WINDOW_FOCUS: {
            auto wf_ev = dynamic_cast<const WindowFocusEvent *>(event);
