...
void SDLRenderImpl::new_frame()
{
    SDL_RenderClear( renderer );
}

void SDLRenderImpl::present_frame()
{
...
void SDLRenderImpl::draw_line( const Vec4& rect )
{
    SDL_RenderLine( renderer, rect.x, rect.y, rect.w, rect.h );
}
...
void SDLRenderImpl::draw_point( float x, float y )
{
    SDL_RenderPoint( renderer, x, y );
}

void SDLRenderImpl::draw_texture(
    const Image* img, const Vec4& p_dest, const Vec4& p_src, float angle, const Color& color
)
{
    auto src  = SDL_FRect{ p_src.x, p_src.y, p_src.w, p_src.h };
    auto dest = SDL_FRect{ p_dest.x, p_dest.y, p_dest.w, p_dest.h };
    SDL_RenderTextureRotated(
        renderer, get_or_upload_img( img ), p_src.is_zero() ? nullptr : &src, p_dest.is_zero() ? nullptr : &dest, angle,
...
SDL_Texture* SDLRenderImpl::get_or_upload_img( const Image* image )
{
    ...
    SDL_Surface* surface = SDL_CreateSurfaceFrom(
        image->width, image->height, SDL_PIXELFORMAT_RGBA32, const_cast<uint8_t*>( image->data.data() ),
    ...
    NC_LOG_TRACE_C(
        log::GRAPHICS, "Cached texture for image at {} ({} bytes)", static_cast<const void*>( image ),
