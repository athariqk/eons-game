...
public:
    ...
    Ref<IRenderSurface> create_surface( void*, Vec2 ) override
    {
        NC_LOG_ERROR_C( log::GRAPHICS, "SDLRenderDevice::create_surface is not implemented" );
        return nullptr;
    }

    RID create_texture( uint32_t, uint32_t, const void* ) override { return RID(); }
    RID create_pipeline( const PipelineDesc& ) override { return RID(); }
    RID create_buffer( BufferType, size_t, const void*, bool ) override { return RID(); }
    void destroy_resource( RID ) override {}

    void* get_native_texture_view( RID ) override { return nullptr; }
    void* get_native_handle() const override { return nullptr; }
    RID get_white_texture() const override { return RID(); }

    void render_2d( IRenderSurface& ) override {}
    void render_3d( IRenderSurface& ) override {}
