...
void register_video_plugin( Scene& scene )
{
    ...
    auto push_material_gpu = []( RenderService& renderer, MaterialComponent& mat ) {
        if (!mat.Instance)
            return;

        const uint32_t dirty = mat.Dirty != MATERIAL_DIRTY_NONE ? mat.Dirty : MATERIAL_DIRTY_ALL;

        if (dirty & MATERIAL_DIRTY_DRAW_MODE)
            renderer.material_set_draw_mode( mat.Instance, mat.DrawMode );

        if (dirty & MATERIAL_DIRTY_TEXTURES) {
            const int count = mat.TextureCount > 0 ? mat.TextureCount : 1;
            for (int i = 0; i < count; ++i) {
                const RID tex = ( i < mat.TextureCount ) ? mat.Textures[i] : RID();
                renderer.material_set_texture( mat.Instance, tex, static_cast<uint32_t>( i ) );
            }
        }

        mat.Dirty = MATERIAL_DIRTY_NONE;
    };
    ...
    scene.get_ecs()
        ...
        .each( [push_material_gpu]( EcsIterState& it ) {
            auto vid    = it.world().get_singleton<VideoServices>();
            ...
            mat->Dirty = MATERIAL_DIRTY_ALL;
            push_material_gpu( *vid->Renderer, *mat );
        } );
    ...
    scene.get_ecs()
        ...
        .each( [push_material_gpu]( EcsIterState& it ) {
            auto vid = it.world().get_singleton<VideoServices>();
            auto mat = it.get_component<MaterialComponent>();
            if (!mat->Instance || mat->Dirty == MATERIAL_DIRTY_NONE)
                return;
            push_material_gpu( *vid->Renderer, *mat );
        } );
...
void register_inputs_plugin( Scene& scene )
{
    ...
    scene.get_ecs()
        ...
        .each( []( EcsIterState& it ) {
            ...
            input->Direction = Vec3( -xz.x, -y, xz.y );
