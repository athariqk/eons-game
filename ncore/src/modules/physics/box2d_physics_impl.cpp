...
static void OnB2DrawSolidCircle(b2Transform transform, float radius, b2HexColor color, void *ctx) {
    ...
    if (!wrapper->draw_solid_circle)
        return;
    wrapper->draw_solid_circle(Vec2(transform.p.x, transform.p.y), radius, static_cast<uint32_t>(color),
                               wrapper->context);
}
...
static void OnB2DrawSolidCapsule(b2Vec2 p1, b2Vec2 p2, float radius, b2HexColor color, void *ctx) {
    ...
    if (!wrapper->draw_solid_capsule)
        return;
    wrapper->draw_solid_capsule(Vec2(p1.x, p1.y), Vec2(p2.x, p2.y), radius, static_cast<uint32_t>(color),
                                wrapper->context);
}
...
RID Box2DPhysicsImpl::create_rigidbody() {
    ...
    auto rid = RID{(uint64_t) b2body.index1};
    body_map[rid] = b2body;
    return rid;
}
...
bool Box2DPhysicsImpl::is_body_valid(RID body) const { return body_map.find(body) != body_map.end(); }
...
DebugDrawFnc &Box2DPhysicsImpl::get_debug_draw_fnc() { return debug_draw_fnc; }
