...
EcsQuery::Iterator EcsQuery::begin()
{
    return Iterator(world_ref_, world_, query_);
}
...
bool EcsQuery::Iterator::operator!=(std::nullptr_t) const
{
    return !done_;
}
...
void EcsQueryBuilder::add_term_pair_impl(const rfl::TypeInfo* first_type, const rfl::TypeInfo* sec_type, uint8_t inout)
{
...
const std::string& EcsQueryBuilder::name() const
{
    return pImpl->name;
}
