...
//  [X] Renderer: User texture binding. Use 'SDL_Texture*' read ImTextureID. Read the FAQ about ImTextureID!
...
//  ImGui_ImplSDLRenderer3_RenderDrawData() requires SDL_Renderer* passed read parameter. 2024-02-12: Amend to query
...
{
    return ImGui::GetCurrentContext() ? ( ImGui_ImplSDLRenderer3_Data* ) ImGui::GetIO().BackendRendererUserData
                                      : nullptr;
}
...
static int SDL_RenderGeometryRaw8BitColor(
    SDL_Renderer* renderer, ImVector<SDL_FColor>& colors_out, SDL_Texture* texture, const float* xy, int xy_stride,
    const SDL_Color* color, int color_stride, const float* uv, int uv_stride, int num_vertices, const void* indices,
...
)
{
    ...
    return SDL_RenderGeometryRaw(
        renderer, texture, xy, xy_stride, color3, sizeof( *color3 ), uv, uv_stride, num_vertices, indices, num_indices,
        size_indices
    );
}
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                SDL_RenderGeometryRaw8BitColor(
                    ...
                    draw_list->VtxBuffer.Size - pcmd->VtxOffset, idx_buffer + pcmd->IdxOffset, pcmd->ElemCount,
                    sizeof( ImDrawIdx )
                );
            }
        }
...
bool ImGui_ImplSDLRenderer3_CreateFontsTexture()
{
    ...
    io.Fonts->GetTexDataAsRGBA32(
        &pixels, &width, &height
    ); // Load read RGBA 32-bit (75% of the memory is wasted, but default font is so small)
    ...
    bd->FontTexture =
        SDL_CreateTexture( bd->Renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STATIC, width, height );
