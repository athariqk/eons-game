...
WindowService::WindowService() : pImpl( std::make_unique<Impl>() ) {}
...
void WindowService::set_default_icon( const Ref<Image>& image )
{
...
void WindowService::window_set_parent( uint32_t window_id, uint32_t parent ) const
{
...
void WindowService::window_set_centered( uint32_t window_id ) const
{
    auto window = pImpl->get_sdl_window( window_id );
...
void WindowService::window_set_visible( uint32_t window_id, bool visible ) const
{
    auto window  = pImpl->get_sdl_window( window_id );
    bool success = false;
    if (visible) {
...
void WindowService::window_set_icon( uint32_t window_id, const Image& image ) const
{
    auto window = pImpl->get_sdl_window( window_id );
...
void WindowService::window_set_fullscreen( uint32_t window_id, bool fullscreen )
{
    auto window = pImpl->get_sdl_window( window_id );
...
void WindowService::window_clear_mouse_confinement( uint32_t window_id ) const
{
    auto window = pImpl->get_sdl_window( window_id );
...
void WindowService::window_set_mouse_grab( uint32_t window_id, bool grabbed ) const
{
    auto window = pImpl->get_sdl_window( window_id );
...
bool WindowService::window_get_mouse_locked( uint32_t window_id ) const
{
    auto window = pImpl->get_sdl_window( window_id );
    return SDL_GetWindowRelativeMouseMode( window );
}

void WindowService::window_set_mouse_locked( uint32_t window_id, bool enabled ) const
{
    auto window = pImpl->get_sdl_window( window_id );
    SDL_SetWindowRelativeMouseMode( window, enabled );
}

bool WindowService::window_pop( uint32_t window_id )
{
...
uint32_t WindowService::get_main_window_id() const
{
    return pImpl->window_ids.empty() ? -1 : pImpl->window_ids.front();
}

void WindowService::set_cursor_type( CursorType cursor_type )
{
...
void WindowService::set_cursor_visible( bool visible )
{
    if (visible) {
        SDL_ShowCursor();
    } else {
...
bool WindowService::get_cursor_visible() const
{
...
void WindowService::pump_events()
{
    event_queue.clear();
