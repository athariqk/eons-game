...
public:
    ...
    T &operator[](uint32_t i) { return pages[i >> page_shift][i & page_mask]; }
    ...
    uint32_t get_page_count() const { return pages.size(); }

    // Returns the size of each page in the arena
    uint32_t get_page_capacity() const { return page_capacity; }
